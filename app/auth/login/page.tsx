    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 lg:space-y-8 p-10 h-full flex flex-col justify-center">
        ...
        <div>
          ...
          <Link href={`/auth/signup?redirectTo=${params.get("redirectTo") || '/explore'}`} className='text-center text-lg block lg:text-2xl'>Don't have an account? Sign up</Link>
          <Link href='/auth/forgot-password' className='text-center text-lg block lg:text-2xl'>Forgot Password?</Link>
        </div>
      </form>
    </Form>
  );
