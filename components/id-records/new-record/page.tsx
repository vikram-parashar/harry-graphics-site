...
import { handleRowDelete, handleDataInsert, handleDataUpdate } from '@/lib/actions/sheets'
...
export default function NewRecord({ sheetId, oldEnties, columns }: {
  ...
  return (
    <div className="container mx-auto p-4">
      <AddRecord sheetId={sheetId} columns={columns} />
      <div className="grid md:grid-cols-3 gap-3 mt-10">
        {oldEnties.map((entry: any, index: number) => (
          <EditRecord key={index} sheetId={sheetId} entry={entry} columns={columns} trigger='img'/>
        ))}
      </div>
    </div>
  )
}
export const AddRecord = ({ sheetId, columns }: {
  sheetId: string,
  columns: SheetType['columns'],
}) => {
...
export const EditRecord = ({ sheetId, entry, columns, trigger }: {
  sheetId: string,
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogTrigger asChild>
          <button className="bg-rosePine-surface border-rosePine-highlightLow p-2 rounded-xl flex gap-5 items-center w-full py-3">
            ...
            {trigger === 'img' && thumbnailTxt(entry)}
          </button>
        </DialogTrigger>
        <DialogContent className='bg-rosePine-base border-rosePine-highlightLow text-rosePine-text'>
          <DialogHeader>
...
export const DeleteRecordAlert = ({ entry, sheetId }: {
  entry: SheetType['data'][0],
  sheetId: string
}) => {
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button variant="ghost" className='hover:text-white hover:bg-rosePine-overlay' > <X /> </Button>
      </AlertDialogTrigger>
      <AlertDialogContent className="dark text-rosePine-text ">
        ...
        <AlertDialogFooter>
          ...
          <Button onClick={() => handleRowDelete(entry, sheetId)} >Continue</Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
