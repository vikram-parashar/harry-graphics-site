...
export default function UpdteCarousel({ item }: { item: Tables<'carousels'> }) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    if (selectedFile) {
      const res = await uploadImage('carousels', selectedFile, null)
      if (res.path) {
        image.path = res.path
      } else {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="bg-background max-h-screen overflow-y-scroll">
        ...
        <DialogTitle className="text-2xl p-2">
          <Highlighter>Update Carousel</Highlighter>
        </DialogTitle>
        <Form {...form}>
          <form
