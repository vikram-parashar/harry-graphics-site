...
export default function CreateCarousel() {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    const image = await uploadImage('carousels', selectedFile, 50);

    if (image.path) {
      const res = await createCarousel(data.title, data.link, data.points?.split(';') || [], image.path)
      if (res.success) {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="bg-background">
        ...
        <DialogTitle className="text-2xl p-2">
          <Highlighter color="#EEBA58">Create Carousel</Highlighter>
        </DialogTitle>
        <Form {...form}>
