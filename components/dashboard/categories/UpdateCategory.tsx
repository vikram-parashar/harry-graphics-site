...
const headingOptions = [
  'ID Solutions', 'Lanyard Solutions', 'Merch', 'Awards', 'Others'
]
...
export default function UpdateCategory({ item }: {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    const image = { path: item.thumbnail_image }
    if (selectedFile) {
      const res = await uploadImage('categories', selectedFile, 50);
      if (res.path) {
        image.path = res.path
      } else {
    ...
    const res = await updateCategory(item.id, data.name, data.heading, image.path)
    if (res.success) {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="bg-background max-h-screen overflow-y-scroll">
        ...
        <DialogTitle className="text-2xl p-2">
          <Highlighter color="#EEBA58">Update Category</Highlighter>
        </DialogTitle>
        <Form {...form}>
            ...
            <FormItem>
              <div className="w-full h-48 relative rounded-lg overflow-hidden">
                <Image
                  ...
                  src={selectedFile ? URL.createObjectURL(selectedFile) : item.thumbnail_image}
