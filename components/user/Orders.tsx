...
export default function Orders({ orders }: { orders: Tables<'orders'>[] }) {
  return (
    <div className="flex flex-col gap-6 p-4 md:p-8">
      <h1 className="text-5xl my-5 p-2">
        <Highlighter>Your Orders</Highlighter>
      </h1>
      {orders.map((order) => (
...
function OrderCard({ order }: { order: Tables<'orders'> }) {
  ...
  return (
    <Card className="bg-secondary-background border-3">
      <CardHeader className="gap-2 text-background">
        ...
        <CardDescription>
          <p className="flex items-center text-xs gap-1 mb-5">
      ...
      <CardContent className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 overflow-hidden">
          <div className="flex items-center justify-between mb-2 text-background">
            <h4 className="font-medium">
              Items ({(order.cart as CartItemType[]).length})
            </h4>
            ...
            {(order.cart as CartItemType[]).map((item, index) => (
              <li key={index} className="bg-background rounded-xl border-2 p-2">
                ...
                <div className="whitespace-nowrap">
                  {money(item.product.price)}x{item.quantity} ={' '}
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border-2 p-4 bg-background">
            <h5 className="mb-2 text-sm font-semibold tracking-tight">
              Shipping to
            </h5>
      ...
      <CardFooter className="flex gap-5">
        ...
        {order.tracking_link ? (
          ...
          <Button
            variant="noShadow"
            className="relative text-background top-1"
            asChild
          >
